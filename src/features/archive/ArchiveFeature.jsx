...
const ARCHIVE_DATA = [
  ...
  { id:9,  title:'출장신청서',                     category:'양식/서식', content:'국내/해외 출장 신청 및 정산 양식입니다.',                       author:'김민준', date:'2025-03-15', files:[{name:'출장신청서.docx',size:'35KB'},{name:'출장정산서.xlsx',size:'30KB'}] },
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:0 }}>
      ...
      <div className="detail-section">
        <div className="detail-section-label">내용</div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            {data.files.map((f, i) => (
              <div key={i} className="file-item">
                ...
                <button className="download-btn" onClick={() => toast.info(`"${f.name}" 다운로드를 시작합니다.`)}>
...
export default function ArchiveFeature({ onRegister, onEdit, onDelete }) {
  ...
  useEffect(() => {
    if (drawerMode === 'register') { reset(INIT_FORM); setFormFiles([]) }
    else if (drawerMode === 'edit' && drawerData) {
      reset({ title: drawerData.title ?? '', category: drawerData.category ?? '', author: drawerData.author ?? '', content: drawerData.content ?? '' })
      setFormFiles([])
    }
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // return archiveApi.getList(applied)  → { list: [], totalCount: 0 }
      // ─────────────────────────────────────────────────────────────────
  ...
  const colDefs = useMemo(() => [
    ...
    {
      field:'files', headerName:'파일', width:70, flex:0, sortable:false, filter:false,
      cellRenderer: ({ value }) => (
        <span style={{ display:'inline-flex', alignItems:'center', gap:4, fontSize:12, color: value?.length ? 'var(--color-accent)' : 'var(--color-text-muted)' }}>
          <Paperclip size={11} />{value?.length ?? 0}
        </span>
      ),
    },
      ...
      <div style={{ flex:1, overflow:'hidden', display:'flex' }}>
        ...
        >
          ...
          {(drawerMode === 'register' || drawerMode === 'edit') && (
            <ArchiveForm control={control} files={formFiles} onFilesChange={setFormFiles} />
          )}
        </FormDrawer>
      </div>
      <ConfirmModal
